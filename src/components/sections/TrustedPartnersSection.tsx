...
const TrustedPartnersSection = () => {
  ...
  return (
    <section className="py-16 bg-charcoal-light border-y border-gold/10">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10">
          <span className="text-gold font-medium uppercase tracking-wider text-sm">
            Trusted By
          </span>
          <h2 className="text-2xl md:text-3xl font-serif font-bold text-primary-foreground mt-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {partners.map((partner) => (
            <div
              key={partner.name}
              className="flex flex-col items-center text-center p-6 rounded-lg bg-primary/50 border border-gold/10 hover:border-gold/30 transition-colors"
            >
        ...
        <p className="text-center text-primary-foreground/50 text-sm mt-8">
          From startups to established corporations, we provide dependable legal guidance across industries.
        </p>
      </div>
    </section>
  );
